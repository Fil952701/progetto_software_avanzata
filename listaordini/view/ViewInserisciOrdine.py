...
class ViewInserisciOrdine(QWidget):
    def __init__(self, controller_lista_ordini, controller_lista_prodotti, update_ui, lista_dinamica):
        ...
        # Lista: E' un sottoinsieme di lista_prodotti, le modifiche fatte su questa lista influenzeranno lista_prodotti solo dopo aver chiamato inserisci_ordine ed aver quindi salvato.
    ...
    # Parte Statica
        ...
        self.tableWidget.setColumnWidth(0, int(self.width/7.72))
        self.tableWidget.setColumnWidth(1, int(self.width/7.72))
        self.tableWidget.setColumnWidth(2, int(self.width/7.72))
        self.tableWidget.setColumnWidth(3, int(self.width/7.72))
        self.tableWidget.setColumnWidth(4, int(self.width/7.72))
        self.tableWidget.setColumnWidth(5, int(self.width/7.72))
        ...
        self.gridLayout.addItem(spacerItem13, 18, 0, 1, 1)
        ...
        self.gridLayout.addItem(spacerItem14, 21, 0, 1, 1)
        ...
        self.gridLayout.addItem(spacerItem15, 24, 0, 1, 1)
        self.label_strumenti_totali = QtWidgets.QLabel(self)
        sizePolicy = QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.Preferred, QtWidgets.QSizePolicy.Fixed)
        sizePolicy.setHorizontalStretch(0)
        sizePolicy.setVerticalStretch(0)
        sizePolicy.setHeightForWidth(self.label_strumenti_totali.sizePolicy().hasHeightForWidth())
        self.label_strumenti_totali.setSizePolicy(sizePolicy)
        self.label_strumenti_totali.setMinimumSize(QtCore.QSize(0, 20))
        self.label_strumenti_totali.setMaximumSize(QtCore.QSize(16777215, 30))
        self.label_strumenti_totali.setObjectName("label_strumenti_totali")
        self.gridLayout.addWidget(self.label_strumenti_totali, 25, 0, 1, 1)
    ...
    def retranslateUi(self):
        ...
        # Testo oggetti dell'interfaccia
        ...
        # I campi importo_totale e strumenti_totali dipendono dai campi quantita e prezzo di acquisto in lista_prodotti_ordine
        importo_totale = 0
        strumenti_totali = 0
        for prodotto in self.lista_prodotti_ordine:
            importo_totale = importo_totale + int(prodotto.prezzo_acquisto)
            strumenti_totali = strumenti_totali + int(prodotto.quantita)
        ...
        self.label_strumenti_totali.setText(_translate("Form", "Strumenti totali: {}".format(str(strumenti_totali))))

    def show_inserisci_prodotto(self):
        inserimento_da_ordine= True

        # in ViewInserisciProdotto alcuni dati per istanziare il prodotto vengono presi dall'ordine in questione
        # Ordine fittizio per inizializzare tutto a zero
        ordine_fittizio= Ordine(0000, 0000, None, None, None, None, None, None)

        self.view_inserisci_prodotto= ViewInserisciProdotto(self.controller_lista_prodotti, self.retranslateUi, inserimento_da_ordine, self.lista_prodotti_ordine, None, ordine_fittizio)
    ...
    def inserisci_ordine(self):
        ...
        if len(self.lista_prodotti_ordine)==0:
            QMessageBox.critical(self, 'Errore', "L'ordine è vuoto e non contiene alcun prodotto.",
                                 QMessageBox.Ok, QMessageBox.Ok)
            return
        ...
        # istanzio l'ordine corrente
        ordine= Ordine(cod_fattura, cod_fornitore, stato,
                      data_ordine1, data_arrivo_prevista, data_arrivo_effettiva,
                      importo_totale, strumenti_totali)

        self.controller_lista_ordini.inserisci_ordine(ordine)
        self.lista_dinamica_ordini.append(ordine)

        # sovrascrivo i campi dei prodotti con quelli dell'ordine
        for prodotto in self.lista_prodotti_ordine:
