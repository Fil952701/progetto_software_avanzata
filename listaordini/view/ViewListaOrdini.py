...
from ordine.view.ViewOrdine import ViewOrdine
from listaordini.view.ViewInserisciOrdine import ViewInserisciOrdine


class ViewListaOrdini(QWidget):
    def __init__(self, parent=None):
        super(ViewListaOrdini, self).__init__(parent)

        self.controller_lista_prodotti = ControllerListaProdotti()
        self.controller_lista_ordini= ControllerListaOrdini()
        # Variabili flag che consentono di tenere monitorati i vari ordini
        self.in_arrivo = False
        self.in_negozio = False
        self.lista_ordini = self.controller_lista_ordini.get_lista_ordini()

        # Stessa cosa effettuata precedentemente -> lista temporanea che viene utilizzata come ausilio per gestire e non cancellare gli ordini
    ...
    # Parte Dinamica
        ...
        self.tableWidget.setColumnWidth(0, int(self.width/14))
        self.tableWidget.setColumnWidth(1, int(self.width/14))
        self.tableWidget.setColumnWidth(2, int(self.width/7.25))
        self.tableWidget.setColumnWidth(3, int(self.width/7.25))
        self.tableWidget.setColumnWidth(4, int(self.width/7.25))
        self.tableWidget.setColumnWidth(5, int(self.width/7.25))
        self.tableWidget.setColumnWidth(6, int(self.width/7.25))
    ...
    def retranslateUi(self):
        ...
        item.setText(_translate("Form", "Strumenti totali"))
        ...
        for ordine in self.lista_dinamica_ordini:
            ...
            self.tableWidget.setItem(row, 5, QtWidgets.QTableWidgetItem(str(ordine.strumenti_totali)))
    ...
    def show_ordine(self):
        if len(self.tableWidget.selectedIndexes()) > 0:
            ...
            self.view_ordine = ViewOrdine(ordine_selezionato, self.retranslateUi, self.controller_lista_ordini, self.controller_lista_prodotti, self.lista_dinamica_ordini)
            self.view_ordine.showMaximized()
            time.sleep(0.3)

    def show_inserici_ordine(self):
        self.view_inserisci_ordine = ViewInserisciOrdine(self.controller_lista_ordini, self.controller_lista_prodotti, self.retranslateUi, self.lista_dinamica_ordini)
        self.view_inserisci_ordine.showMaximized()

    # Metodi per filtrare le varie indicazioni

    # Indicazione al filtro generale filter() per filtrare ordini in arrivo
    ...
    # Indicazione al filtro generale filter() per filtrare ordini in negozio
    ...
    # Indicazione al filtro generale filter() per visualizzare tutti gli ordini
    def filter_all(self):
        self.in_arrivo = False
        self.in_negozio = False
        self.lista_ordini = self.controller_lista_ordini.get_lista_ordini()
    ...
    # Indicazione al filtro generale sugli ordini con cod_fattura contenente il testo immesso
    ...
    # Filtro generale che svolge più filtraggi
    def filter(self):
        elementi_da_rimuovere = []
