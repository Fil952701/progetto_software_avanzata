...
class ControllerFornitore:
    ...
    # metodi di recupero dati per il controller fornitore
    ...
    def get_stato(self):
        if self.model.stato == "S":
            return "Standard"
        elif self.model.stato == "P":
            return "Premium"
        else:
            raise Exception()

    # metodi di settaggio dati per il controller fornitore

    def set_cod_fornitore(self, codice_fornitore):
        if codice_fornitore.startswith("TH") and codice_fornitore.isalnum() and len(codice_fornitore) < 6:
    ...
    def set_stato(self, stato):
        stati_validi = ["Standard", "Premium"]
        if stato in stati_validi:
            self.model.stato = "S" if stato == "Standard" else "P"
        else:
            raise ValueError("Valore dello stato non valido. Deve per forza essere 'Standard' o 'Premium'.")