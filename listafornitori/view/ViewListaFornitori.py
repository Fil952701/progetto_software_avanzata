...
from fornitore.view.ViewFornitore import ViewFornitore
from listafornitori.controller.ControllerListaFornitori import ControllerListaFornitori
from listafornitori.view.ViewInserisciFornitore import ViewInserisciFornitore

class ViewListaFornitori(QWidget):
    def __init__(self):
        super(ViewListaFornitori, self).__init__()
        ...
        # Lista -> E' una copia temporanea della lista reale. Mi permette di manipolare i suoi elementi senza modificare la lista originale (lista di riferimento)
    ...
    def show_inserisci_fornitore(self):
        self.vista_inserisci_fornitore= ViewInserisciFornitore(self.controller, self.retranslateUi, self.lista_dinamica)
    ...
    def show_fornitore(self):
        ...
        if(len(self.tableWidget.selectedIndexes()) > 0):
            ...
            self.vista_fornitore = ViewFornitore(fornitore_selezionato, self.retranslateUi, self.controller, self.lista_dinamica)
            self.vista_fornitore.show()

    # Indicazione al filtro generale filter() che vogliamo filtrare fornitori premium
    ...
    # Indicazione al filtro generale filter() che vogliamo filtrare fornitori standard
    ...
    # Indicazione al filtro generale filter() che vogliamo visualizzare tutti i fornitori
    def filter_all(self):
        self.standard= False
        self.premium= False
        self.lista_fornitori = self.controller.get_lista_fornitori()
    ...
    # Filtraggio dei fornitori con cod_fornitore contenente il testo immesso
    ...
    # Come creato precedentemente per gli utenti, filtro generale che svolge più filtraggi
    def filter(self):
        elementi_da_rimuovere = []
